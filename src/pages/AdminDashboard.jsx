...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen flex">
      {/* Sidebar - Integrated with Portfolio aesthetic */}
      <aside className="w-72 bg-black/40 backdrop-blur-md border-r border-white/5 p-10 flex flex-col">
        <div className="flex items-center gap-4 mb-16">
          <img src="/img/logo.png" className="h-10 w-10 object-contain" alt="Logo" />
          <h1 className="text-sm text-white font-black uppercase tracking-widest leading-tight">
            Tolibas Iron Works
          </h1>
        </div>
        ...
        <button onClick={() => { supabase.auth.signOut(); navigate('/login'); }} className="text-left text-sm font-black uppercase tracking-widest text-red-900 hover:text-red-600">
          Log Out
        </button>
      </aside>
        ...
        <div className="w-full max-w-2xl">
          ...
          {activeTab === 'inquiries' && (
            <div className="grid gap-6">
              {inquiries.map(inq => (
                <div key={inq.id} className="p-8 bg-white/5 backdrop-blur-sm border border-white/10 rounded-sm hover:bg-white/10 transition-colors">
                  <h4 className="font-black uppercase text-base text-white tracking-wide">{inq.name}</h4>
                  <p className="text-xs text-zinc-300 mb-4 font-medium">{inq.contact_info}</p>
          ...
          {activeTab === 'add-project' && (
            <form onSubmit={handleAddProject} className="flex flex-col gap-6 bg-white/5 backdrop-blur-sm p-8 border border-white/5 rounded-sm">
              ...
              <div className="flex flex-col gap-3">
                <label className="text-[10px] font-black uppercase text-zinc-500 tracking-widest">Registry Category</label>
                <div className="grid grid-cols-2 gap-3">
                  {['Gates', 'Staircases', 'Windows', 'Structural'].map((cat) => (
                    <label key={cat} className="flex items-center gap-2 text-xs font-bold text-zinc-300 cursor-pointer hover:text-white">
              ...
              <input type="file" accept="image/*" required className="w-full p-4 bg-transparent border border-white/10 text-sm text-zinc-400 outline-none file:mr-4 file:bg-white file:border-none file:px-4 file:py-2 file:text-[10px] file:font-black" onChange={e => setFile(e.target.files[0])} />
              
              <button disabled={loading} className="w-full bg-white text-black font-black py-4 text-xs uppercase tracking-widest hover:bg-zinc-200 transition-all">
          ...
          {activeTab === 'view-projects' && (
            <div className="grid grid-cols-2 gap-6">
              {projects.map(proj => (
                <div key={proj.id} className="bg-white/5 backdrop-blur-sm border border-white/5 p-4 rounded-sm">
                  ...
                  <button onClick={() => handleDeleteProject(proj.id)} className="text-[10px] text-red-500 uppercase mt-2 font-bold hover:text-red-400">Delete Project</button>
                </div>
              ))}
            </div>
          )}
